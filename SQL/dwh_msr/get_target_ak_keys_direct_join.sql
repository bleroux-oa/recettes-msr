WITH lkp_key_lma_sample_duplicates as 
(SELECT
  {version}
...
SELECT s.ak_key_msr_sample, s.ver_lma_sample, s.lkp_key_lma_sample
FROM lkp_key_lma_sample_duplicates
INNER JOIN `rni-sknow-gbl-ww-pd.sknow_bqdset_dwh_msr_eu_pd.tbl_dim_sample` s
USING (lkp_key_lma_sample)